...
function saveGame() {
  ...
  fetch("https://git.heroku.com/buginvadersbackend.git/api/v1/games", {
    method: "POST",
    headers: {
...
function createUser(e) {
  ...
  fetch("https://git.heroku.com/buginvadersbackend.git/api/v1/users", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json"
...
function fetchComments() {
  fetch("https://git.heroku.com/buginvadersbackend.git/api/v1/comments")
    .then(res => res.json())
    .then(data => {
...
function allTimeScoresIteration(games) {
  ...
  for (let i = 0; i < 10; i++) {
    const li = document.createElement("li")
    li.innerText =
      bestScores[i].score + " - " + bestScores[i].name.split(" ")[0]
